...
serve(async (req) => {
  ...
  try{
    ...
    // Ventas en rango
    const { data: ventas } = await sb.from('ventas').select('*').gte('fecha_hora', fromDate.toISOString()).lte('fecha_hora', toDate.toISOString())
    const totalVendido = (ventas || []).reduce((s:any, v:any) => s + parseFloat(v.total), 0)
    ...
    // Gastos en rango
    const { data: gastos } = await sb.from('gastos').select('*').gte('fecha_hora', fromDate.toISOString()).lte('fecha_hora', toDate.toISOString())
    const totalGastos = (gastos || []).reduce((s:any, g:any) => s + parseFloat(g.monto), 0)
    ...
    if(ventaIds.length > 0){
      ...
      for(const d of (detalles || [])){
        prodMap[d.producto_id] = (prodMap[d.producto_id] || 0) + parseInt(d.cantidad)
      }
      ...
      if(prodIds.length > 0){
        ...
        productosTop.sort((a,b)=> b.cantidad - a.cantidad)
      }
    }
